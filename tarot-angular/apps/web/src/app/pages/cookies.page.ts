...
export default class CookiesPage implements OnInit {
  private readonly seo = inject(SeoService);
  private readonly tarotState = inject(TarotStateService);

  readonly content = legalContent[this.tarotState.language()].cookies;

  ngOnInit() {
    this.seo.setArticleMeta(this.content.title, this.content.sections[0]?.paragraphs[0] ?? "", "/cookies");
  }
}
